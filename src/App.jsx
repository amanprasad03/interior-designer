...
import { Upload, Search, DollarSign, FileText, AlertCircle, Loader2, Settings, CheckCircle, Key, Eye, EyeOff, ExternalLink, Shield, Trash2 } from 'lucide-react';
  ...
  const validateAndSaveApiKey = async () => {
    ...
    try {
      ...
      if (testResponse.ok) {
        ...
        setError(`API key validation failed: ${errorData.error?.message || 'Invalid API key'}`);
      }
    } catch (err) {
          ...
          const priceResponse = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: "user",
                  content: `Based on typical 2025 market prices, estimate the cost per ${item.unit} for: ${item.material} - ${item.item_name}.
...
DO NOT OUTPUT ANYTHING OTHER THAN VALID JSON.`
          ...
          if (priceResponse.ok) {
            ...
            priceText = priceText.replace(/```json\n?/g, "").replace(/```\n?/g, "").trim();
      ...
      const totalCost = itemsWithPrices.reduce((sum, item) => sum + (item.total_cost || 0), 0);
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  <span className="text-xs text-slate-500 ml-auto">console.anthropic.com</span>
                </a>
                <p className="text-xs text-slate-500 mt-2">
                  Sign up or log in, then create a new API key in the API Keys section
                </p>
              </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-slate-600">
                Upload 2D interior drawings to automatically extract materials and generate cost estimates
              </p>
              {apiKeySet && (
                  ...
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-semibold text-green-800 mb-1">API Key Active</h4>
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-sm text-yellow-800">
                      <strong>No API key configured.</strong> You need an Anthropic API key to use this application.
                    </p>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <label className="block w-full border-2 border-dashed border-slate-300 rounded-lg p-8 text-center cursor-pointer hover:border-blue-500 transition-colors">
                ...
                <p className="text-slate-600 mb-1">Click to upload interior drawing</p>
            ...
            {imagePreview && (
              <div className="bg-white rounded-xl shadow-lg p-6">
                <h3 className="text-lg font-semibold text-slate-800 mb-4">Drawing Preview</h3>
                <img 
                  src={imagePreview} 
                  alt="Drawing preview" 
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="mb-4 p-4 bg-slate-50 rounded-lg">
                  ...
                  <p className="text-sm text-slate-600">
                    <strong>Rooms:</strong> {analysisResult.rooms?.join(', ') || 'Not specified'}
                  </p>
                  <p className="text-sm text-slate-600 mt-2">
                    <strong>Total Items:</strong> {analysisResult.items?.length || 0}
                  </p>
                </div>
                  ...
                  {analysisResult.items?.map((item, idx) => (
                    <div key={idx} className="border border-slate-200 rounded-lg p-4 hover:bg-slate-50 transition-colors">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${getCategoryColor(item.category)} mb-2`}>
                            {item.category}
                          </span>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div key={idx} className="border border-slate-200 rounded-lg p-4">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-slate-600">{item.material}</p>
                          <p className="text-xs text-slate-500 mt-1">
                            {item.quantity} {item.unit} × ${item.unit_price?.toFixed(2) || '0.00'}/{item.unit}
                          </p>
                          {item.assumptions && (
                            <p className="text-xs text-slate-400 mt-2 italic">{item.assumptions}</p>
                          )}
                        </div>
                        <div className="text-right">
                ...
                <div className="border-t border-slate-200 pt-4 space-y-2">
                  <div className="flex justify-between text-slate-700">
                    <span>Subtotal:</span>
                    <span className="font-semibold">${costEstimate.subtotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-slate-700">
                    <span>Contingency (15%):</span>
                    <span className="font-semibold">${costEstimate.contingency.toFixed(2)}</span>
                  </div>
                ...
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <p className="text-xs text-yellow-800">
                    <strong>Note:</strong> Prices are estimates based on typical 2025 market rates. 
                    Actual costs may vary based on location, supplier, and specifications.
                  </p>
                </div>
              </div>
            )}
